...
import { buildEnhancedSystemPrompt } from '@/lib/system-prompts';
      ...
      try {
        ...
        if (isSurgicalEdit) {
          ...
          if (manifest) {
            ...
            try {
              const intentResponse = await fetch(`${origin}/api/analyze-edit-intent`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
          ...
          if (editContext) {
            ...
            if (global.activeSandbox) {
              ...
              try {
                ...
                const filesResponse = await fetch(`${origin}/api/get-sandbox-files`, {
                  method: 'GET',
                  headers: { 'Content-Type': 'application/json' }
                });
...
Remember: You are a SURGEON making a precise incision, not an artist repainting the canvas!`,
        ...
        // Build system prompt with conversation awareness using enhanced prompt builder
        const morphFastApplyEnabled = Boolean(isSurgicalEdit && process.env.MORPH_API_KEY);

        let systemPrompt = buildEnhancedSystemPrompt({
          isEdit,
          isSurgicalEdit,
          isPlannedBuild: isPlannedBuildStep,
          buildProfile,
          editContext: editContext ? {
            primaryFiles: editContext.primaryFiles,
            editIntent: {
              type: editContext.editIntent?.type || 'MODIFICATION',
              description: editContext.editIntent?.description || 'User-requested change',
            }
          } : undefined,
          conversationContext,
          morphFastApply: morphFastApplyEnabled,
        });

        console.log('[generate-ai-code-stream] Using enhanced system prompt builder');

        // If Morph Fast Apply is enabled (surgical edits only + MORPH_API_KEY), add morph instructions
